...
def solve():
    ...
    def dfs(x, y, height, visited, k, count):
        global max_count
        ...
        for dir in range(4):
            ...
            if visited[nx][ny] == 0 and height > maps[nx][ny] - k:       # 방문하지 않은 위치 and 현재 높이보다 낮은 높이를 방문
                if maps[nx][ny] >= height:    # 다음 위치의 높이가 현재 위치보다 크다면 현재 위치보다 1 낮게 깎음
                    nh = height - 1
                    # print(visited)
                    visited[nx][ny] = 1           # 방문 기록
                    dfs(nx, ny, nh, visited, 0, count + 1)
                    visited[nx][ny] = 0           # 방문 초기화
                else:
                    nh = maps[nx][ny]
                    # print(visited)
                    visited[nx][ny] = 1           # 방문 기록
                    dfs(nx, ny, nh, visited, k, count + 1)
                    visited[nx][ny] = 0           # 방문 초기화


    for i, j, height, k, c in locs:
        visited = [[0] * N for _ in range(N)]
        dfs(i, j, height, visited, k, c)
