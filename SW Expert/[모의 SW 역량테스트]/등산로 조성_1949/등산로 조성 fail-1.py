...
def solve():
    ...
    for i, j, h, k, c in locs:
        ...
        q = list()
        q.append((i, j, h, k, c, visited))
        visited[i][j] = 1
        # print()
        # print(q)

        while q:
            x, y, h, k, count, t_visited = q.pop()
            ...
            for dir in range(4):
                ...
                if nx < 0 or ny < 0 or nx >= N or ny >= N:
                    continue
                if t_visited[nx][ny] == 0 and h > maps[nx][ny] - k:       # 방문하지 않은 위치 and 현재 높이보다 낮은 높이를 방문
                    t_visited[nx][ny] = 1      # 방문 기록
                    if maps[nx][ny] >= h:    # 다음 위치의 높이가 현재 위치보다 크다면 현재 위치보다 1 낮게 깎음
                        nh = h - 1
                        q.append((nx, ny, nh, 0, count + 1, t_visited))
                    else:
                        nh = maps[nx][ny]
                        q.append((nx, ny, nh, k, count + 1, t_visited))
                    t_visited[nx][ny] = 0      # 방문 기록 초기화? (다른 경로에서 방문했던 기록을 없애줌)
        #             print(q)
        #             print(visited)
